...
import {
  ShieldCheck, LayoutDashboard, FileCheck2, ListTodo, AlertTriangle,
  Users, KeyRound, Building2, ScrollText, Boxes, Settings2,
  FileBarChart, LogOut, Bell, Briefcase, Sparkles, Sliders,
} from "lucide-react";
...
const NAV: Record<Role, NavItem[]> = {
  owner: [
    { to: "/owner", label: "نظرة تنفيذية", icon: LayoutDashboard },
    { to: "/owner/departments", label: "الأقسام", icon: Building2 },
    { to: "/owner/waste-map", label: "خريطة الهدر", icon: AlertTriangle },
    { to: "/owner/risk-map", label: "خريطة المخاطر", icon: AlertTriangle },
    { to: "/owner/what-if", label: "محاكي القرار", icon: Sliders },
    { to: "/owner/ledger", label: "السجل غير القابل للتعديل", icon: ScrollText },
    { to: "/owner/exports", label: "التقارير والتصدير", icon: FileBarChart },
  ],
  gm: [
    { to: "/gm", label: "لوحة المدير العام", icon: LayoutDashboard },
    { to: "/gm/departments", label: "أداء الأقسام", icon: Building2 },
  ],
  manager: [
        ...
        <header className="h-14 border-b border-border flex items-center justify-between px-6 bg-card/50">
          ...
          <div className="flex items-center gap-3">
            <button className="relative w-9 h-9 rounded-md bg-card border border-border flex items-center justify-center hover:border-primary transition">
              ...
              <span className="absolute -top-1 -left-1 w-4 h-4 rounded-full bg-danger text-[10px] text-white flex items-center justify-center">3</span>
            </button>
          </div>
        </header>
