...
import { ShieldCheck, CheckCircle2, AlertTriangle, Search, Calendar, Filter, Clock } from "lucide-react";
...
const RICH_LEDGER_ENTRIES = [
  { id: "l1", at: "2026-06-28 09:14", actor: "زينب الكاظمي", action: "اعتماد مستند", target: "INV-2026-0481", hash: "a7c361fa2d0391f", department: "المشتريات", timestamp: new Date("2026-06-28T09:14:00") },
  { id: "l2", at: "2026-06-28 09:02", actor: "زينب الكاظمي", action: "تصحيح خطأ OCR", target: "حقل المبلغ: 12,400,000 ← 12,450,000", hash: "b21df0c44ae834a", department: "المشتريات", timestamp: new Date("2026-06-28T09:02:00") },
  { id: "l3", at: "2026-06-27 15:45", actor: "أحمد المياحي", action: "ترحيل قيد تسوية", target: "تسوية حساب مصرف الجنوب الإسلامي حزيران", hash: "f3c3098eab110d2", department: "المالية", timestamp: new Date("2026-06-27T15:45:00") },
  { id: "l4", at: "2026-06-26 11:20", actor: "حيدر الكرخي", action: "مطابقة رصيد مخزن الكرادة", target: "8 أصناف تفاوت - شحنة المواد الأولية ج", hash: "4d76b1e8e2029bf", department: "المخازن", timestamp: new Date("2026-06-26T11:20:00") },
  { id: "l5", at: "2026-06-25 08:31", actor: "مصطفى الجبوري", action: "منح صلاحية استثنائية مؤقتة", target: "view_waste_map ← مدير المشتريات (7 أيام)", hash: "9e88cb2cd4420c2", department: "المالية", timestamp: new Date("2026-06-25T08:31:00") },
  { id: "l6", at: "2026-06-24 18:00", actor: "النظام", action: "كشف هدر مالي آلي", target: "تكرار فاتورة INV-2026-9001 (12.4M)", hash: "81e2b3c4f5a6d7e", department: "المشتريات", timestamp: new Date("2026-06-24T18:00:00") },
  { id: "l7", at: "2026-06-22 02:00", actor: "النظام", action: "تشغيل التحليل اليومي", target: "مؤشر الثقة = 78 / 100", hash: "5ff17c093a027b9", department: "عام / النظام", timestamp: new Date("2026-06-22T02:00:00") },
  { id: "l8", at: "2026-06-15 14:10", actor: "سارة حسن", action: "تحديث شروط التعاقد", target: "مورد السكر المعتمد - مصنع الفرات", hash: "7c3b91a2e30f1d9", department: "المشتريات", timestamp: new Date("2026-06-15T14:10:00") },
  { id: "l9", at: "2026-06-10 10:00", actor: "النظام", action: "تهيئة البيئة السحابية للشركة", target: "حاجب البيانات الذاتي RLS نشط بنجاح", hash: "d3b2f5c719e83fa", department: "عام / النظام", timestamp: new Date("2026-06-10T10:00:00") }
];
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle={locale === "ar" ? "سلسلة SHA-256 مشفرة وروابط غير قابلة للتعديل أو الحذف — إثبات فوري لسلامة الأرقام." : "تۆمارێکی متمانەپێکراو"}
        action={
          <button
            onClick={() => {
              setVerified(null);
              setTimeout(() => {
                setVerified(true);
                setMessage(locale === "ar" ? "تم التحقق تشفيرياً: السجل سليم 100%" : "تۆمارەکە ١٠٠٪ سەلامەتە");
              }, 800);
            }}
      ...
      {verified === true && (
        <div className="p-4 rounded-xl bg-success/10 border border-success/30 text-success flex items-center gap-2 animate-fade-in text-xs font-medium">
          ...
          <span>{message} — تم مطابقة {filteredEntries.length} قيد مالي مع البصمة الأصلية.</span>
        </div>
      )}
        ...
        <div className="flex flex-col md:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder={locale === "ar" ? "البحث باسم المدقق، الإجراء، رقم الفاتورة أو الهاش..." : "گەڕان..."}
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <span>{sortOrder === "newest" ? (locale === "ar" ? "الأحدث أولاً" : "نوێترین") : (locale === "ar" ? "الأقدم أولاً" : "کۆنترین")}</span>
            </button>
          </div>
        </div>
      ...
      <div className="rounded-2xl bg-card border border-border overflow-hidden shadow-sm">
        {filteredEntries.length === 0 ? (
          <div className="p-12 text-center text-muted-foreground space-y-2">
            ...
            <p className="text-sm font-medium">{locale === "ar" ? "لا توجد سجلات مطابقة لمعايير البحث والفلترة حالياً." : "هیچ ئەنجامێک نەدۆزرایەوە."}</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-muted/80 text-muted-foreground text-xs font-bold uppercase tracking-wider">
                <tr>
                  <th className="text-right p-4">{locale === "ar" ? "الوقت والتاريخ" : "کات و ڕێکەوت"}</th>
                  <th className="text-right p-4">{locale === "ar" ? "المبادر / المستخدم" : "بەکارھێنەر"}</th>
                  <th className="text-right p-4">{locale === "ar" ? "الإجراء المحاسبي" : "کردار"}</th>
                  <th className="text-right p-4">{locale === "ar" ? "الهدف والمستند" : "ئامانج"}</th>
                  <th className="text-right p-4">{locale === "ar" ? "القسم" : "بەش"}</th>
                  <th className="text-right p-4">{locale === "ar" ? "هاش التشفير (SHA-256)" : "هاش"}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border/60">
                {filteredEntries.map((e) => (
                  <tr key={e.id} className="hover:bg-muted/30 transition-all">
                    ...
                    <td className="p-4 text-muted-foreground text-xs max-w-[280px] truncate" title={e.target}>
                      {e.target}
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4 font-mono text-xs text-primary/80 whitespace-nowrap" dir="ltr">
                      {e.hash}
                    </td>
                  </tr>
                ))}
