...
export const Route = createFileRoute("/login")({
  head: () => ({ meta: [{ title: "AuditCore Login" }] }),
  component: Login,
});
      ...
      <div className="hidden lg:flex flex-col justify-between p-12 bg-sidebar text-sidebar-foreground border-l border-sidebar-border">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              منصة الذكاء التدقيقي السيادي / پلاتفۆرمی ژیری پشکنینی سەربەخۆ
            </div>
          </div>
        </div>
        <div className="space-y-6">
          ...
          <p className="text-muted-foreground leading-loose max-w-md">
            صلاحيات صارمة، سجل لا يُمحى، وتحليل صامت يكشف الهدر والمخاطر — دون أن يطّلع المدقق على
            أي استنتاج مالي.
          </p>
        </div>
        ...
        <div className="w-full max-w-md space-y-8">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <h2 className="text-2xl font-bold">{t("auth", "title", locale)}</h2>
            </div>
            <p className="text-sm text-muted-foreground">{t("auth", "subtitle", locale)}</p>
          </div>
          <form onSubmit={submit} className="space-y-4">
            <div>
              <label className="text-sm font-medium block mb-2">{t("auth", "email", locale)}</label>
              <input
                type="email"
                required
                value={email}
            ...
            >
              {loading ? t("auth", "loading", locale) : t("auth", "submit", locale)}
            </button>
          </form>
          <div className="pt-6 border-t border-border">
            <div className="text-xs text-muted-foreground mb-3">الحسابات التجريبية</div>
